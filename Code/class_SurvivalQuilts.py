...
class SurvivalQuilts(): 
    ...
    def train(self, X, T, Y):
        ...
        self.all_time_horizons  = [t for t in range(int(np.min(T[Y.iloc[:,0] == 1])) + 1, int(np.max(T[Y.iloc[:,0] == 1])))]
        ...
        for k in range(self.K):
            ...
            for out_itr in range(self.num_outer):
                ...
                for itr in range(self.num_bo):
                    print('k: %d/%d, out_itr: %d/%d, itr: %d/%d' % \
                        (k+1, self.K, out_itr+1, self.num_outer, itr+1, self.num_bo))
