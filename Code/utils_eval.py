...
def load_csv_dataset(dataset_path, label_mode):
    df_raw = pd.read_csv(dataset_path)
    print("Loading data from %s with %d samples." % (dataset_path, df_raw.shape[0]))
    
    train_labels = ["Age recode", "Sex", "Year of diagnosis", "Race recode", \
                    "Primary Site", "Grade", "SRC", "Tstage", "Nstage", \
                    "Mstage", "Surgery", "Regional nodes examined", \
                    "Neoadjuvant therapy", "Radiation", "Chemotherapy"]

    X = df_raw[train_labels]
    Y = df_raw[[label_mode]]
    T = df_raw[['Survival months']]

    return X, Y, T

def get_eval_time_ratio(eval_time_horizon, T):
    sum_cnt = T.shape[0]
    L1 = (T < eval_time_horizon).sum()[0]
    L2 = (T == eval_time_horizon).sum()[0]
    return (L1 + L2 / 2.0) / sum_cnt